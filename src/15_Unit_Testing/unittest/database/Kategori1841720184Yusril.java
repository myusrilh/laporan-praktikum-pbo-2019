...
public class Kategori1841720184Yusril {
    ...
    public ArrayList<Kategori1841720184Yusril> getByNamaAndKeterangan(String nama, String keterangan) {
        ArrayList<Kategori1841720184Yusril> ListKategori = new ArrayList();

        ResultSet rs;

        if (nama.trim().length() > 0 && keterangan.trim().length() > 0) {
            rs = DBHelper1841720184Yusril.selectQueryYusril("SELECT * FROM kategori"
    ...
    public String messageFormatYusril(){
        String message = String.format("Buku %s memiliki keterangan %s", this.nama, this.keterangan);
        return message;
    }
